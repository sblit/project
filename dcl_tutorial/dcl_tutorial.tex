...
address has joined. Unconnected packets are sent over a DCL network and between
two addresses -- thus between two network endpoints on the same network.
...
callbacks upon joining of DCL networks (\code{onJoin()}), receipt of unconnected
packets (\code{onReceive()}) and incoming application channel requests
(\code{onApplicationChannelRequest()}).
